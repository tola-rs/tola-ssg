...
fn url_path(path_prefix: &Path, filename: &str) -> String {
    let prefix = normalize_path_prefix(path_prefix);
    if prefix.is_empty() {
        format!("/{TOLA_DIR}/{filename}")
    } else {
        format!("/{prefix}/{TOLA_DIR}/{filename}")
    }
}

fn normalize_path_prefix(path_prefix: &Path) -> String {
    path_prefix
        .iter()
...
impl EmbeddedAsset<()> {
    ...
    /// URL path for this asset (e.g., "/prefix/.tola/enhance.css").
    pub fn url_path(&self, path_prefix: &Path) -> String {
        url_path(path_prefix, &self.filename())
    }
    ...
    #[allow(dead_code)]
    pub fn external_tag(&self, path_prefix: &Path) -> String {
        let url = self.url_path(path_prefix);
        match self.kind {
...
impl<V: TemplateVars> EmbeddedAsset<V> {
    ...
    /// URL path for this rendered asset (e.g., "/prefix/.tola/spa-a1b2c3d4.js").
    pub fn url_path_with_vars(&self, path_prefix: &Path, vars: &V) -> String {
        url_path(path_prefix, &self.filename_with_vars(vars))
    }
    ...
    pub fn external_tag_with_vars(&self, path_prefix: &Path, vars: &V) -> String {
        let url = self.url_path_with_vars(path_prefix, vars);
        match self.kind {
            AssetKind::Css => format!(
                r#"<link rel="stylesheet" href="{}" {}>"#,
                url, GENERATED_ATTR
...
mod tests {
    ...
    #[test]
    fn url_path_uses_generated_root_for_empty_prefix() {
        let asset = EmbeddedAsset::new(AssetKind::Css, "enhance", "");

        assert_eq!(asset.url_path(Path::new("")), "/.tola/enhance.css");
    }

    #[test]
    fn url_path_normalizes_nested_prefix() {
        let asset = EmbeddedAsset::new(AssetKind::JavaScript, "spa", "");
        let prefix = PathBuf::from("docs").join("blog");

        assert_eq!(asset.url_path(&prefix), "/docs/blog/.tola/spa.js");
    }
}
